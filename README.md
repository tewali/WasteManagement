...
- **Persistence:** the JSON file store (`.data/`) lives on a 1 GB persistent
  disk mounted at `/opt/render/project/src/.data`, so uploads and
  verifications survive deploys. Disks require a paid instance type; to trial
  on the **free** tier, delete the `disk` block from `render.yaml` (data then
